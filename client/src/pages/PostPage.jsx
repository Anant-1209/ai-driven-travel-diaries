      ...
      <div className='flex justify-between items-center w-full max-w-2xl mx-auto'>
        ...
        {currentUser && (
          <button 
            ...
            className='mt-5 flex items-center gap-2 text-slate-500 hover:text-teal-500 transition-colors'
          >
            {isSaved ? <HiBookmark className='w-6 h-6 text-teal-500' /> : <HiOutlineBookmark className='w-6 h-6' />}
            <span className='text-sm font-medium'>{isSaved ? 'Saved' : 'Save Diary'}</span>
          </button>
        )}
      </div>
      <img
